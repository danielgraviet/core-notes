...
## Acceptance Criteria
- [ ] List shows note `title` as the primary label
- [ ] List shows first ~60 characters of `body` as a secondary line (truncated, no newlines)
- [ ] List is sorted by `modifiedAt` descending (newest first)
- [ ] A `searchText` state variable filters the list using a `@Query` predicate — not client-side `.filter()`
- [ ] Empty state: when no notes exist, show a simple "No notes" message (no heavy placeholder UI)
- [ ] "Add Note" toolbar button creates a `Note` with empty title/body and immediately navigates to the detail view
- [ ] Delete still works via swipe/Edit mode
- [ ] Keyboard shortcut `Cmd+N` creates a new note
